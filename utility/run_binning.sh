...
# 
##############################################################################################################################################################
SOFT=/home/adityans/scriptome/metaWRAP/bin/metawrap-scripts

help_message () {
	echo ""
	echo "Usage: binning [options] -a assembly.fa -o output_dir readsA_1.fastq readsA_2.fastq ... [readsX_1.fastq readsX_2.fastq]"
...
metabat1=false; metabat2=false; maxbin2=false; solidbin=false;
checkm=false; read_type=paired
markers=107

# load in params
OPTS=`getopt -o ht:i:m:o:a:l: --long help,metabat1,metabat2,maxbin2,solidbin,run-checkm,single-end,universal,interleaved --`
...
if [ $metabat2 = false ] && [ $metabat1 = false ] &&[ $maxbin2 = false ] && [ $solidbin = false ]; then
	help_message
	error "You must select at least one binning method: --metabat2, --metabat1, --maxbin2, --solidbin"
fi

# check if all parameters are entered
if [ $out = false ] || [ $in = false ]|| [ $ASSEMBLY = false ] ; then 
	comm "Non-optional parameters -i, -a and/or -o were not entered"
	help_message; exit 1
fi
...
if [ $read_type = paired ]; then
	...
	num_of_F_read_files=$(for I in $files; do echo $I | grep _1.fastq; done | wc -l)
	num_of_R_read_files=$(for I in $files; do echo $I | grep _2.fastq; done | wc -l)
	...
	if [ $read_type = paired ]; then
		if [[ $num == *"_1.fastq"* ]]; then 
			...
			if [[ ! -f ${out}/work_files/${sample}.bam ]]; then
				...
				minimap2 -ax sr -t $threads ${out}/work_files/assembly.fa $reads_1 $reads_2 > ${out}/work_files/${sample}.sam
		...
		if [[ $num == *".fastq"* ]]; then
			...
			if [[ ! -f ${out}/work_files/${sample}.bam ]]; then
				...
				if [ $read_type = single ]; then
					minimap2 -ax sr -t $threads ${out}/work_files/assembly.fa $reads > ${out}/work_files/${sample}.sam
...
if [ $solidbin = true ]; then
	########################################################################################################
	########################                    RUNNING SOLIDBIN                     ########################
	########################################################################################################
    announcement "RUNNING SOLIDBIN"
	assembly=${out}/work_files/assembly.fa
	mapdir=${out}/work_files
	samtools faidx ${out}/work_files/assembly.fa;
	awk -v OFS='\t' {'print $1,$2'} ${assembly}.fai > ${mapdir}/length.txt;
...
do
    stub=${file%.sam}
    echo $stub  
    samtools view -h -b -S $file > ${stub}.bam; samtools view -b -F 4 ${stub}.bam > ${stub}.mapped.bam;samtools sort -m 1000000000 ${stub}.mapped.bam -o ${stub}.mapped.sorted.bam; bedtools genomecov -ibam ${stub}.mapped.sorted.bam -g ${mapdir}/length.txt > ${stub}_cov.txt 
done
...
Collate.pl $mapdir > ${mapdir}/coverage.tsv

perl -pe "s/,/\t/g;" ${mapdir}/coverage.tsv > ${mapdir}/coverage_new.tsv
gen_kmer.py $assembly 1000 4 
