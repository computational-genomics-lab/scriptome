...
class clusterDAT(luigi.Task):
	...
	def run(self):
		...
		cmd_run_corset = "[ -d  {corset_folder} ] || mkdir -p {corset_folder}; cd {corset_folder}; " \
						 ...
						 "-i salmon_eq_classes {salmon_quant_folder}*/aux_info/eq_classes.txt -f true " \
...
class denovoTransAssemblyDEA(luigi.Task):
	adapter = GlobalParameter().adapter
	organism_domain = GlobalParameter().organism_domain
	read_library_type = GlobalParameter().read_library_type
	...
	def requires(self):
		return [clusterDAT(rnaseq_assembler=self.rnaseq_assembler,project_name=self.project_name,
						   pre_process_reads=self.pre_process_reads)]
	...
	def output(self):
		...
		resultFolder = os.path.join(os.getcwd(), self.project_name,
									"denovo_assembly_based_dea",
									"DEAnalysis",
									self.dea_method + "_" + self.rnaseq_assembler + "_" +  self.result_tag + "_" + self.read_library_type + "/")
